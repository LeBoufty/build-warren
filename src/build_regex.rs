...
pub const HEADER_REGEX: &str = r#"/<div class="page-header "><h1>(.+)\((T|P|Z)v(T|P|Z|X) (Cheese|All-In|Timing Attack|Economic|Co-op|None)\)<\/h1>/gm"#;

// Regex pattern to parse the build order's details
pub const DETAILS_REGEX: &str = r#"/<h4>Details<\/h4><ul>(?:<li>Created by: ([^\s]+) ? ?<\/li>)(?:<li>Published on: ([^<]+|) ? ?<\/li>)(?:<li>Modified on: (?:[^<]+|)<\/li>)(?:<li>Patch: ([^<]+|)<\/li>)(?:<li>Difficulty: ([^<]+|)<\/li>)?/gm"#;

// Regex pattern to parse the build order's VOD
pub const VOD_REGEX: &str = r#"/<h3 id="vod-header">VOD<\/h3><a href="([^"]+)" target=/gm"#;

// Regex pattern to parse the build order's votes
pub const VOTES_REGEX: &str = r#"/<span>Votes&nbsp;<\/span>(?:<small>No votes<\/small>|<span class="[^"]+">(\d{1,3})%<\/span>&nbsp;<small>(\d+) votes<\/small>)/gm"#;

// Regex pattern to parse the build order's description
pub const DESCRIPTION_REGEX: &str =
    r#"/<h3 id="description-header">Description<\/h3><p>(.*(?!<\/p>)*)<\/p>/gm"#;
